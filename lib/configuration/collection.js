...
function wrapArray1(parent, query, containedType) {
  ...
  conf.forEach((cls) => {
    ...
    cls.wrapReadOnlyProperty('Size', 'length', {
      name: 'Size',
      key: 'Size()',
      arguments: [],
      return: { type: 'Standard_Integer' }
    });
    cls.wrapConstructor({
      name: cls.nativeName.split('_')[1],
      key: `${cls.nativeName.split('_')[1]}(Standard_Integer, Standard_Integer)`,
      arguments: [
    ...
    cls.wrapConstructor({
      name: cls.nativeName.split('_')[1],
      key: `${cls.nativeName.split('_')[1]}(v8::Array)`,
      arguments: [{ name: 'Array', type: 'v8::Array' }],
      return: { type: cls.nativeName }
    }, 'jsArrayConstructorOverload');
  });
