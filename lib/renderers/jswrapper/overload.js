...
class MethodOverloadRenderer extends base.Renderer {
  ...
  renderGetNativeArgs() {
    return this.def.args.map((arg, i) => {
  ...
  renderOverloadFunctions() {
    const getArgs = this.renderGetNativeArgs().join('\n');
    const checkForArgErrors = !getArgs ? '' : `\
      if(errorHandler.HasCaught()){
        return false;
      }`;
...
class JSArrayConstructorOverloadRenderer extends ConstructorOverloadRenderer {
  renderNativeCall() {
    var wrappedArgType = this.typemap.getWrappedType(this.parentClass().containedType);
    var getArg = wrappedArgType.toNative('res', 'jsarray->Get(Nan::New(i))',
    ...
    return `\
      ...
      auto jsarray = maybeArray.ToLocalChecked().As<v8::Array>();
      auto collection = new ${this.parentClass().nativeName}(1, jsarray->Length() + 1);
      for (int i = 0; i < jsarray->Length(); i++) {
        ${this.parentClass().containedType} res;
        ${getArg}
        collection->SetValue(i + 1, res);
      }

      auto wrapper = new ${this.parentClass().qualifiedName}(collection);
      wrapper->Wrap(info.This());`;
  }
  returnType() {
    return this.parentClass().nativeName;
...
JSArrayConstructorOverloadRenderer.prototype.declType = 'jsArrayConstructorOverload';
...
class IndexedGetterOverloadRenderer extends GetterOverloadRenderer {
  getArgNames() {
    return ['index + 1'].concat(super.getArgNames());
  }
...
module.exports = {
  ...
  JSArrayConstructorOverloadRenderer
};
