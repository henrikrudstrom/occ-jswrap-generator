...
class IndexedGetterRenderer extends CallableRenderer {
  constructor(def, factory, typemap) {
    super(def, factory, typemap);
    this.nanMacro = 'NAN_INDEX_GETTER';
  }

  renderMemberInitialization() {
    var setterArg = this.def.setterName ? `, ${this.def.setterName}` : '';
    return `Nan::SetIndexedPropertyHandler(ctorInst, ${this.def.name}${setterArg});`;
  }
}
IndexedGetterRenderer.prototype.declType = 'indexedGetter';


class IndexedSetterRenderer extends CallableRenderer {
  constructor(def, factory, typemap) {
    super(def, factory, typemap);
    this.nanMacro = 'NAN_INDEX_SETTER';
  }
}
IndexedSetterRenderer.prototype.declType = 'indexedSetter';
...
module.exports = {
  ...
  GetterRenderer,
  SetterRenderer,
  IndexedGetterRenderer,
  IndexedSetterRenderer
};
